...
serve(async (req) => {
  ...
  if (!record || record.saving_balance > record.saving_target) {
  ...
  form.append("html", `
    <p>Greetings from PlanIT! This is a gentle reminder that your savings account balance has dropped below your saving target. 
    Top up your saving account by navigating to  PlanIT Dashboard -> Saving -> Set Saving / Add Saving.</p>
    <ul>
      <li><strong>Saving Balance:</strong> $${record.saving_balance}</li>
      <li><strong>Saving Target:</strong> $${record.saving_target}</li>
    </ul>
  `);
